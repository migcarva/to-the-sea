      ...
      this.DOM.facts = {
        ...
        items: Array.from(
          this.DOM.el.querySelectorAll(
            '.section__facts > .section__facts-item',
          ),
        ),
      };
    }
  ...
  class Slideshow {
    ...
    layout() {
      ...
      for (let i = 0; i <= this.entriesTotal - 1; ++i) {
        ...
        entry.DOM.expander.style.transform = `scale3d(0.54,1,1) translate3d(0px,${
    ...
    initEvents() {
      ...
      // Main menu
      this.DOM.menu.menuCtrls.toggle.addEventListener('click', () =>
        this.toggleMenu(),
      );
    ...
    toggleMenu() {
      ...
      const toggleMenuCtrlFn = () => {
        anime.remove([
          this.DOM.menu.menuCtrls.open,
          this.DOM.menu.menuCtrls.close,
        ]);
        return anime({
          targets: [
            this.DOM.menu.menuCtrls.open,
          ...
          opacity: (target, index) =>
            index ? (!this.isMenuOpen ? 1 : 0) : !this.isMenuOpen ? 0 : 1,
